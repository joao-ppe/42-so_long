...
/*   Updated: 2023/12/18 18:25:03 by joao-ppe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_enemy(t_game *game)
{
	...
	while (i < game->enemy_n)
	{
		...
		if (move == 0 && move_enemy_up(game, i) == -1
			&& validate_enemy_move(game, game->enemies[i].x, game->enemies[i].y - 1) == 0)
			continue ;
		else if (move == 1 && move_enemy_down(game, i) == -1
			&& validate_enemy_move(game, game->enemies[i].x, game->enemies[i].y + 1) == 0)
			continue ;
		else if (move == 2 && move_enemy_left(game, i) == -1
			&& validate_enemy_move(game, game->enemies[i].x - 1, game->enemies[i].y) == 0)
			continue ;
